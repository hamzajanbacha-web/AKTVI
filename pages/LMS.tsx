...
import { User, ClassroomParticipant, ChatMessage, Quiz, DiscussionPost } from '../types';
...
const StudentsCornerView: React.FC<{ user: User; dbData: any }> = ({ user, dbData }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
       <div className="bg-slate-50 border-b border-teal-100 p-4 flex items-center justify-center gap-3 overflow-x-auto scrollbar-hide">
          {folds.map(fold => (
            <button 
              key={fold.id} 
       ...
       <div className="flex-grow p-12 animate-fadeIn overflow-y-auto">
          {activeFold === 'hub' && (
             <div className="max-w-4xl mx-auto space-y-12">
                <div className="text-center">
                   <h2 className="text-5xl font-black text-teal-950 uppercase tracking-tighter">Salaam, {user.firstName || user.username}</h2>
                   <p className="text-teal-600 font-urdu text-3xl mt-4">تعلیمی پورٹل میں خوش آمدید۔</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   <div className="bg-teal-50 p-10 rounded-[3rem] text-center border border-teal-100 shadow-sm"><p className="text-5xl font-black text-teal-900">45%</p><p className="text-[11px] font-black text-teal-400 mt-3 uppercase tracking-widest">Syllabus Status</p></div>
                   <div className="bg-amber-50 p-10 rounded-[3rem] text-center border border-amber-100 shadow-sm"><p className="text-5xl font-black text-amber-600">{user.points || 0}</p><p className="text-[11px] font-black text-amber-400 mt-3 uppercase tracking-widest">Academy Gems</p></div>
                   <div className="bg-emerald-50 p-10 rounded-[3rem] text-center border border-emerald-100 shadow-sm"><p className="text-5xl font-black text-emerald-600">{user.badges?.length || 0}</p><p className="text-[11px] font-black text-emerald-400 mt-3 uppercase tracking-widest">Merit Badges</p></div>
                </div>
             </div>
          )}
       </div>
...
const LMS: React.FC<{ externalUser: User | null; onLogout: () => void; onPageChange: (p: string) => void }> = ({ externalUser, onLogout, onPageChange }) => {
